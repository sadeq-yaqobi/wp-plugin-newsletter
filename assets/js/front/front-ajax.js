jQuery(document).ready(function ($) {
        ...
        let email = $('#nl_email').val();
                // AJAX request to filter posts
                        $.ajax({
                            ...
                            dataType: 'json',
                            data: {
                                action: 'nl_news_letter_subscribe',
                                email:email,
    ...
    //handle unsubscription form
    $('.unsubs-form').on('submit', function (e) {
        e.preventDefault();
        let email = $('#unsubs_email').val();
        // AJAX request to filter posts
        $.ajax({
            url:nl_ajax.nl_ajaxurl , //ajax url
            type: 'post',
            dataType: 'json',
            data: {
                action: 'nl_news_letter_unsubscribe',
                email:email,
                _nonce:nl_ajax._nl_nonce
            },
            beforeSend: function () {
                $('.unsubs-btn').html('<div class="nl-loader"></div>');
            },
            success: function (response) {
                if (response.success) {
                    $.toast({
